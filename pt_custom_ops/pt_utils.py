...
class MaskedQueryAndGroup(nn.Module):
    ...
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, features=None):
        # print("-0-")
        # print(support_xyz.shape)
        # print(features.shape)
        # print(torch.sum(support_mask,dim=1))
        # print("-1-")
        # print(query_xyz.shape)
        # print(torch.sum(query_mask,dim=1))
        # for bi in range(support_xyz.shape[0]):
        #     write_ply("./NaN_B{:02d}_0.ply".format(bi),
        #     [support_xyz[bi,...].detach().cpu(),support_mask[bi,...].detach().cpu()],["vertex","m"])
        #     write_ply("./NaN_B{:02d}_1.ply".format(bi),
        #     [query_xyz[bi,...].detach().cpu(),query_mask[bi,...].detach().cpu()],["vertex","m"])
            
        idx, idx_mask = masked_ordered_ball_query(self.radius, self.nsample, query_xyz, support_xyz,
                                                  query_mask, support_mask)
        # the two lines below are added to deal with rare special cases
        idx[idx>support_xyz.shape[1]] = 0
        idx[idx<0] = 0
        # print("-b-")
        # print(idx.shape)
        # print(idx[:,0,:])
        # print(torch.sum(idx_mask,dim=1))
        # print("---")
