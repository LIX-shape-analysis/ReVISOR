...
class OutlierSegmentationDataset(data.Dataset):
    def __init__(self, input_features, katz_params, katz_type, subsampling_parameter,
        ...
        assert dataset_type in ["other","EDFM","EDFS","EDFS3f0","EDFS3f1","EDFS3f2","EDFL","PCN"]
        ...
        num_classes_file = os.path.join(self.data_dir, '_num_inliers_num_outliers_{}.pkl'.format(self.split))
        ...
        if os.path.exists(num_classes_file):
            ...
            print("[LOADED] {:012d} INLIERS AND {:012d} OUTLIERS IN SPLIT {}".format(num_inliers,num_outliers,self.split))
        else:
            num_inliers = 0
            num_outliers = 0
            for file in files:
            ...
            print("[COMPUTED] {:012d} INLIERS AND {:012d} OUTLIERS IN SPLIT {}".format(num_inliers,num_outliers,self.split))
        ...
        for cloud_idx, cloud_name in enumerate(self.cloud_names):
            ...
            cluster_file = os.path.join(self.data_dir, "1NN_{:.2f}".format(subsampling_parameter), cloud_name + '.ply.npy')
    ...
    def init_datasets(self):
        if self.dataset_type=="PCN":
            ...
            with open(os.path.join(self.data_root,'trainingset.txt'), 'r') as f:
                self.train_clouds = file_of_files_to_list(f)

            with open(os.path.join(self.data_root,'validationset.txt'), 'r') as f:
                self.val_clouds = file_of_files_to_list(f)

            with open(os.path.join(self.data_root,'testset.txt'), 'r') as f:
                self.test_clouds = file_of_files_to_list(f)
    ...
    def __getitem__(self, idx):
        ...
        if self.input_features_dim==0:
            current_features = current_points.clone().numpy() # force conversion from torch to numpy due to train_dist code
        else:
        ...
        return [current_points, mask, features,
                       current_points_labels, current_cloud_index, input_inds, point_ind]
...
if __name__=="__main__":
    for diam_perc in [10.,20.,40.]:
        dset = OutlierSegmentationDataset(data_root=edf_root,input_features=[], katz_params=[], katz_type='std', in_radius=0.5*diam_perc*10./100., subsampling_parameter=0.04, num_points=15000,
                    num_steps=7, num_epochs=5, split='train', transforms=transforms,dataset_type="EDF",DEBUG=False)
